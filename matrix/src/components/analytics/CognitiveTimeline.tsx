          ...
          {(d) => (
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="h-72 border border-border/70 rounded-sm p-2 bg-zinc-950/40">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={monthlyChart}>
                      ...
                      <XAxis dataKey="month" stroke="#71717a" fontSize={9} tickLine={false} interval={5} />
                      <YAxis yAxisId="vol" stroke="#71717a" fontSize={10} tickLine={false} />
                      <YAxis yAxisId="rate" orientation="right" domain={[40, 100]} stroke="#71717a" fontSize={10} tickLine={false} tickFormatter={(v: number) => `${v}%`} />
                      ...
                      <Line yAxisId="rate" type="monotone" dataKey="rate" stroke="#34d399" strokeWidth={1.5} dot={false} connectNulls isAnimationActive={false} />
                      <Line yAxisId="rate" type="monotone" dataKey="rolling" stroke="#fbbf24" strokeWidth={1.5} strokeDasharray="5 3" dot={false} connectNulls isAnimationActive={false} />
                    </ComposedChart>
                  </ResponsiveContainer>
                </div>
              </div>
                  ...
                  <div className="h-56 border border-border/70 rounded-sm p-2 bg-zinc-950/40">
                    <ResponsiveContainer width="100%" height="100%">
                      <ComposedChart data={d.monthOfYear.map((m) => ({ ...m, label: MONTH_LABELS[m.month - 1], pct: m.rate !== null ? +(m.rate * 100).toFixed(1) : null }))}>
                        ...
                        <YAxis yAxisId="pct" domain={[40, 100]} stroke="#71717a" fontSize={10} tickLine={false} tickFormatter={(v: number) => `${v}%`} />
                        <YAxis yAxisId="n" orientation="right" stroke="#52525b" fontSize={9} tickLine={false} />
                        <Tooltip contentStyle={tooltipStyle} cursor={{ fill: "#27272a33" }}
                          formatter={(v: number, name: string) => [name === "pct" ? `${v} %` : v, name === "pct" ? "taux" : "n binaire"]} />
                        <Bar yAxisId="n" dataKey="n" fill="#27272a" isAnimationActive={false} />
                        <Line yAxisId="pct" type="monotone" dataKey="pct" stroke="#f87171" strokeWidth={1.5} dot={{ r: 2 }} isAnimationActive={false} />
                      </ComposedChart>
                    </ResponsiveContainer>
                  </div>
                    ...
                    <tbody>
                      {d.byYear.map((y) => (
                        <tr key={y.year} className="border-b border-border/40 hover:bg-secondary/40">
                          ...
                          <td className={`px-3 py-1.5 ${y.rate >= 0.7696 ? "text-red-400" : "text-emerald-400"}`}>
                            {fmtPct(y.rate)}
                          </td>
                        </tr>
                      ))}
